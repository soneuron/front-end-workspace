...
// last : 선택된 요소들 중 마지막 요소 선택
$("h4:last").css("font-size", "3rem");

// eq(n) : 선택된 요소들 중 인데스 번호와 일치하는 요소 선택(0부터 시작)
// 3번째 요소를 가져와서 "eq에 의해선택되었습니다"로 텍스트 변경(text)
$("h4:eq(2)").text("eq에 의해 선택되었습니다.");

// odd : 홀수 번 째 인덱스에 위치한 요소 선택 (index 1,3,5 ...)
// even : 짝 수 번째 인덱스에 위치한 요소 선택 (index 2,4,6 ...)
// 홀 수 번째 인덱스 요소를 가져와서 배경색 : hotpink, 글자색 : white
...
// parents("선택자") : 선택된 요소의 '모든 상위 요소' 선택를 선택하거나, 매개변수가 있으면 매개변수와 일치하는 상위 요소만 선택
...
// parentsUntil("선택자") : 선택된 요소의 모은 상위 요소를 선택하거나, 매개변수가 있으면 선택된 요소부터 매개변수와 일치하는 요소까지의 상위 요소 선택 (~~까지)
// span요소부터 div 요소 이전까지 상위 요소에 배경색 : lavenderblush
...
// 3. Sideways 메서드
// siblings("선택자") : 선택된 요소와 같은 레벨에 있는 모든 요소를 선택하거나 매개변수가 있으면 같은레벨 요소 중 매개변수와 일치하는 요소 선택
...
// nextAll("선택자") : 선택된 요소와 같은 레벨 요소 중 다음의 모든 요소 선택
// h2 요소의 다음에 오는 모든 형제 요소에 배경색 : pink
$(".wrap2 h2").nextAll().css({ color: "pink" });
...
// prevAll("선택자") : 선택된 요소와 같은 레벨 요소 중 '이전의 모든 요소' 선택
// h2 요소의 이전에 오는 모든 형제 요소에 배경색 : moccasin
$(".wrap2 h2").prevAll().css("background-color", "moccasin");

// prevUntil("선택자") : 선택된 요소와 같은 레벨 요소 중 '이전의 모든 요소'를 선택하거나, 매개변수가 있으면 같은 레벨의 이전 요소 중 매개변수와 일치하는 요소까지 선택
// h2 요소 이전부터 p 요소 이후 까지 모든 형제 요소에 글자크기 : 2rem
